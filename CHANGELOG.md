...
### Added
- **Country flags dropped from the origin line, and the "Trending" heading finally translated.** `_originHtml()` assembled a regional-indicator flag at runtime from the two-letter country code, so no source-level emoji scan ever saw it — it rendered in the OS font (a different visual language from every other icon in the card), degrades to a plain two-letter box on Windows, and is politically loaded for several coffee-growing regions in a way a flat country name is not. Nothing is lost: the resolved country name was always rendered immediately after it. Separately, the trending section heading was hardcoded English in an otherwise fully six-language card; it now resolves through a new `trending_title` key in all six. #90
...
- **Drawn icons (`GLP-SHARED:icons v1`) replace every emoji glyph in the card (#90)** — 38 occurrences across the 6 `STRINGS` languages and the trending/status render paths (12× ☕, 6× 🎉, 6× ✓, 6× ✕, 2× 🔥, plus 6× ⏳ the original count missed: `pending`'s hourglass isn't one of the five icons this redesign ships, and adding a sixth to the shared, byte-identical `GLP-SHARED:icons v1` block mid-round would drift it from glp-lovelace-card's copy — dropped rather than shoehorned into a wrong-fit icon). New `_menuIconHtml()` maps the six known `DEFAULT_MENU` drink ids (espresso/ristretto/lungo/cappuccino/latte/flat_white) to a drawn icon; any other id — a user-created menu entry, `POST/PUT api/orders/menu` — falls back to that entry's own stored `emoji` character, escaped. The `emoji` field itself is untouched: still persisted app-side, still writable, no migration. `test/menu-icon.test.js` covers all three shapes (known id, custom-entry fallback, an entry with neither).
